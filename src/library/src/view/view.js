(function() {
  ...
  function View(controller) {
    ...
    this.$newBookSubmit = this.$newBookForm.querySelector(
      'input[type="submit"]'
    );
    ...
    this.$newBookForm.addEventListener(
      "submit",
      this.__handleNewBookFormSubmit
    );
    ...
    this.__handleRatingContainerClick = function(e) {
      ...
      if (targetStarIndex === currentBook.rating) {
        ...
        this.replaceChild(self.__renderRating(0), this.querySelector("ul"));
      } else {
    ...
    this.$filtersContainer.addEventListener(
      "click",
      this.__handleFiltersContainerClick
    );

    this.__handleMainSearchFormSubmit = function(e) {
      e.preventDefault();
    };
    this.$mainSearchForm.addEventListener(
      "submit",
      this.__handleMainSearchFormSubmit
    );

    this.__handleMainSearchInputInput = function(e) {
      var inputText = this.value;

      var filteredBooksArr = self.controller.getFilterBooksBySearch(
    ...
    this.$mainSearchInput.addEventListener(
      "input",
      debounce(this.__handleMainSearchInputInput)
    );
  }
  ...
  View.prototype.createBook = function(bookObj) {
    ...
    book.innerHTML = `
            ...
            <span class="book__author">by ${bookObj.author.firstName} ${
    ...
    ratingContainer.addEventListener(
      "click",
      this.__handleRatingContainerClick
    );
  ...
  View.prototype.renderNewNoteToHistory = function(noteDetails) {
    ...
    if (noteDetails.event === "rating-change") {
      noteText = `
            ...
            by <b>${noteDetails.author.firstName} ${
        noteDetails.author.lastName
      }</b>
            `;
    } else if (noteDetails.event === "filter") {
      noteText = `
            You filtered books by using <b>${noteDetails.filterName}</b>
            `;
    } else if (noteDetails.event === "search") {
      noteText = `
            You searched books by following words: <b>"${
              noteDetails.searchWords
            }"</b>
            `;
    }
    ...
    if (allLiElements.length !== 3) {
      ...
      this.$sidebarHistory.removeChild(allLiElements[2]);
